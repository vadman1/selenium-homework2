...
public class ParametrizedRaiffeisenTest extends BaseTest {

    @ParameterizedTest
    @ValueSource(strings = {"Петров Петр Петрович","Иванов Иван Иванович", "Фамилия Имя Отчество"})
    public void test(String fullName) {
        ...
        } catch (NoSuchElementException | ElementNotInteractableException | StaleElementReferenceException ignore) {
        } finally {
        ...
        fillInputField(fullNameField, fullName);
        driver.findElement(By.xpath("//div[text()='"+fullName+"']")).click();
